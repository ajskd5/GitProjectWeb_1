...
public class EmpServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 화면 출력 부분
		// 여기에 코딩하는게 JSP => _jspService(){JSP코딩}
		// 1. 변환 => 컴파일 => HTML변환
		response.setContentType("text/html;charset=UTF-8"); // text/html, text/xml, text/plain=>JSON
		PrintWriter out = response.getWriter(); // 사용자 정보
		// out => 클라이언트의 브라우저에서 읽어가는 위치
		// 출력할 데이터 가지고 옴
		EmpDAO dao = new EmpDAO();
		List<EmpVO> list = dao.empListData1();
